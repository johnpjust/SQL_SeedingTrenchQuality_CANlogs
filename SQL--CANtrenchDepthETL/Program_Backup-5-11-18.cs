    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (var context = new ISU_Internal_CANEntities())
            {
                ...
                //this is the query
                //var query = context.can_raw_data_Seeding_2018.Where(b => b.ref_id > 41865 && b.ref_id < 41865+2 ).OrderBy(b=> b.ts_sec).ThenBy(b=>b.ts_usec).GroupBy(b => b.ref_id);
                var query = context.can_raw_data_Seeding_2018.Where(b => b.ref_id > 41885 && b.ref_id < 41900).Select(item => new can_raw_data_seeding_query // && b.ref_id <= 41884
                {
                    ts_sec = item.ts_sec,
                ...
                }).ToList().OrderBy(b => b.ts_sec).ThenBy(b => b.ts_usec).GroupBy(b => b.ref_id);//.Select(s => s.Key);// || b.ref_id == 41866 || b.ref_id == 41867
                ...
                Parallel.ForEach(query, new ParallelOptions { MaxDegreeOfParallelism = 8 }, (q) =>
                {
                    int RUCoffset = 127;
                    int TDoffset = 207;
                    int thisRow = 0;
